...
import { AiServiceUnavailableError } from "../utils/AppError.js";
...
const buildContext = (documents, metadatas, distances) => {
  if (!documents || !documents[0] || documents[0].length === 0) {
    return "";
  }
...
const ragService = {
  ...
  async generateAnswer({ query, ownerId, noteIds, chatHistory = [] }) {
    ...
    try {
      results = await embeddingsService.queryRelevantChunks({
        query,
        ownerId,
        noteIds,
        topK: 5,
      ...
      logger.warn("Vector retrieval failed; continuing without semantic context", {
        error: error.message,
      });
    ...
    if (!context && noteIds && noteIds.length > 0) {
      try {
        ...
        context = notes
          .map(
            (n) =>
              `[Source: ${n.originalFilename}]: ${n.extractedText || n.excerpt || ""}`,
          )
          .join("\n\n");
      } catch (err) {
        logger.warn("Fallback note fetch failed for RAG context", {
          error: err.message,
        });
      }
    }
    ...
    try {
      responseText = await groqService.generateChatResponse(
        systemPrompt,
        formattedHistory,
        query,
      );
      ...
      logger.warn(
        "Groq chat generation failed, retrying with Gemini fallback",
        {
      ...
      try {
        ...
        logger.error("All LLM providers failed for chat generation", {
