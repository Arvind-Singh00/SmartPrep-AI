...
const resolveApiBaseUrl = () => {
  if (API_ORIGIN) return `${API_ORIGIN}/api`;
  if (API_BASE_URL) {
  ...
  if (import.meta.env.PROD) {
    console.warn(
      "SmartPrep-AI: VITE_API_BASE_URL is not configured. Production requests will fall back to /api and may fail if the backend is not mounted there.",
    );
  }
...
axiosClient.interceptors.request.use(
  (config) => {
    const token =
      localStorage.getItem("study_buddy_token") ||
      sessionStorage.getItem("study_buddy_token");
    if (token) {
