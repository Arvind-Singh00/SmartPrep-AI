import { adapter, isMockModeEnabled } from "./_adapter";
...
const chatService = {
  fetchSessions: () => {
    const isReal = !isMockModeEnabled();
  ...
  sendMessage: ({ chatId, text, noteIds }) => {
    ...
    return adapter
      .post(
        ...
        isReal
          ? { query: text, chatId, noteIds: cleanNoteIds }
          : { text, noteIds: cleanNoteIds },
        1200,
      )
      .then((res) => {
